...
const GET = async ({ request }) => {
  ...
  try {
    ...
    let query = supabase.from("credit_transactions").select(`
      ...
      `).order("created_at", { ascending: false }).limit(1e4);
    if (type) {
    ...
    let transactions = (data || []).map((tx) => ({
      id: tx.id,
      user_name: tx.users?.display_name || "",
      user_phone: tx.users?.phone_number || "",
    ...
    return new Response(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="credit-transactions-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv"`
      }
    });
  } catch (error) {
