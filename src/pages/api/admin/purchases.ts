/**
 * Admin API: Purchases list with pagination and filters
 * GET /api/admin/purchases?page=1&search=&type=&status=&from=&to=
 */
...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    let purchases = (data || []).map((p) => ({
      id: p.id,
      user_id: p.user_id,
      user_name: (p.users as any)?.display_name || null,
      user_phone: (p.users as any)?.phone_number || null,
      purchase_type: p.purchase_type,
      pack_name: (p.credit_packs as any)?.display_name || null,
      tier_name: (p.user_subscriptions as any)?.subscription_tiers?.display_name || null,
    ...
    if (search) {
      ...
      purchases = purchases.filter(
        (p) =>
          ...
          p.meshulam_transaction_id?.includes(search)
      );
    }
